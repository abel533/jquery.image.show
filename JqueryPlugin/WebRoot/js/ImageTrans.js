...
ImageTrans.prototype = {
  ...
  reset: function() {
	if ( this._support ) {
		if(this._img.clientWidth>1500&&this._img.clientWidth<1800){
			this._x = this._y = 0.63; this._radian = 0;
		}
		else{
			this._x = this._y = 0.40; this._radian = 0;
		}
		hei =0;
		this._show();
	}
	...
	return {
		css3: {//css3设置
			...
			show: function() {
				...
				if (hei == 130) {
					...
					img.style.top = -2107 + "px";
				}
				else if (hei == 150) {
					img.style.top = -2900 + "px";
				}
				else if (hei == 160) {
					img.style.top = -3500 + "px";
				}
				else if (hei == 170) {
					img.style.top = -4200 + "px";
				}
				else if (hei == 180) {
					img.style.top = -4600 + "px";
				}
			},
		...
		filter: {//滤镜设置
			...
			show: function() {
				...
				//设置滤镜
				$$.extend(
				img.filters.item("DXImageTransform.Microsoft.Matrix"), getMatrix(this._radian, this._y, this._x));
				//保持居中
				if (hei == 0) {
					if (img.offsetHeight > 1600) {

						img.style.top = -1507 + "px";
					}

					if (img.offsetHeight <= 1600) {
						img.style.top = (this._clientHeight - img.offsetHeight) / 2 - 120 + "px";
					}
				}
				if (hei == 130) {
					img.style.top = -200 + "px";
				...
				img.style.left = ( this._clientWidth - img.offsetWidth ) / 2 + 10  + "px";
			},
			dispose: function(){ this._container.removeChild(this._img); }
		},
		canvas: {//canvas设置
